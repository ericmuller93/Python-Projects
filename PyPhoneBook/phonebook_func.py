...
def center_window(self, w, h): #were passing in the tkinter fram (master) reference adn the w and h
    #this will get the users screen w and h
    ...
    return centerGeo
# catch if the user clicks x to close the window to ensure they want to close
def ask_quit(self):
...
def onSelect(self,event):
    #calling the event is the self.lst.List1 widget
...
def addToList(self):
    ...
    var_fname = var_fname.strip() #this will remove an blank spaces before or after the users entry
    var_lname = var_lname.strip()
    var_fname = var_fname.title() #this will ensure the first letter of the names are capitalized
    var_lname = var_lname.title()
    var_fullname = ('{} {}'.format(var_fname,var_lname)) # combines both names to full name
    print('var_fullname: {}'.format(var_fullname))
    var_phone = self.txt_phone.get().strip()
    var_email = self.txt_email.get().strip()
    if(len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): #ensure user input data into all fields
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
            ...
            if chkName == 0: # if this is 0 then there is no existence of fullname and we can add new data
                ...
                messagebox.showerror("Name Error","'{}' already exists in the database! Please choose a different name.".formar(var_fullname))
                onClear(self) #calls the function to clear all textboxes
        conn.commit()
        conn.close()
    else:
...
def onDelete(self):
    var_select = self.lstList1.get(self.lstList1.curselection()) #get listboxes selected value
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
        ...
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delete Confirmation", "All information assocaited with, ({}) \n will be permanantly deleted from the database. \n\nProceed with the deletion request?".format(var_select))
            if confirm:
                conn  = sqlite3.connect('phonebook.db')
                with conn:
                    conn = conn.cursor()
            ...
            confirm = messagebox.showerror("Last Record Error", "({}) is the last record in the database and cannot be deleted at the time. \n\nPlease add another record first before you can delete ({}).".format(var_select,var_select))
    conn.close()
...
def onRefresh(self):
    # populate the list box, coinciding with the database
...
def onUpdate(self):
    ...
    var_phone = self.txt_phone.get().strip() #strips any extra spaces to maintain data intgrity
    var_email = self.txt_email.get().strip()
    if (len(var_phone) > 0 and (len(var_email) > 0)): #ensure there is data present
        ...
        with conn:
            ...
            # the database, meaning check if there are even changes to apply
